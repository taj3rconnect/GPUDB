    ...
    <div class="flex items-center gap-4">
      <div class="text-xs text-gray-500">
        Refresh: <select id="refreshRate" onchange="updateRefreshRate()" class="bg-surface-100 border border-gray-700 rounded px-2 py-1 text-gray-300 text-xs">
          <option value="3000">3s</option><option value="5000">5s</option><option value="10000">10s</option><option value="30000">30s</option><option value="60000">1m</option><option value="300000">5m</option><option value="600000">10m</option><option value="900000">15m</option><option value="3600000">1h</option>
        </select>
  ...
  <div style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:700px;max-width:95vw;max-height:85vh;overflow-y:auto;background:#1e1e2e;border:1px solid #333;border-radius:12px;padding:24px">
    ...
    <div id="addServerForm" style="display:none" class="mt-4 p-4 bg-surface-100 rounded-lg border border-gray-700">
      ...
      <div class="grid grid-cols-3 gap-3">
        <div><label class="text-xs text-gray-400">Server Name <span class="text-red-400">*</span></label><input id="srv_name" class="w-full mt-1 px-2 py-1.5 bg-gray-800 border border-gray-600 rounded text-sm text-white" placeholder="e.g. A100 Cluster"></div>
        <div><label class="text-xs text-gray-400">Host / IP <span class="text-red-400">*</span></label><input id="srv_host" class="w-full mt-1 px-2 py-1.5 bg-gray-800 border border-gray-600 rounded text-sm text-white" placeholder="e.g. 10.0.0.5"></div>
      ...
      <input type="hidden" id="srv_key" value="">
      <details class="mt-3">
        <summary class="text-xs text-gray-400 cursor-pointer hover:text-gray-300">Advanced options</summary>
        <div class="grid grid-cols-3 gap-3 mt-2">
          ...
          <div><label class="text-xs text-gray-400">GPU Model</label><input id="srv_gpu" class="w-full mt-1 px-2 py-1.5 bg-gray-800 border border-gray-600 rounded text-sm text-white" placeholder="auto-detected"></div>
          <div><label class="text-xs text-gray-400">vLLM Ports</label><input id="srv_vllm" class="w-full mt-1 px-2 py-1.5 bg-gray-800 border border-gray-600 rounded text-sm text-white" placeholder="e.g. 8001,8002"></div>
          <div><label class="text-xs text-gray-400">Services</label><input id="srv_services" class="w-full mt-1 px-2 py-1.5 bg-gray-800 border border-gray-600 rounded text-sm text-white" placeholder="e.g. vllm-main,agent"></div>
          <div class="flex items-center gap-4">
            <label class="flex items-center gap-2 text-xs text-gray-300"><input type="checkbox" id="srv_local"> Local</label>
            <label class="flex items-center gap-2 text-xs text-gray-300"><input type="checkbox" id="srv_ecc"> ECC</label>
          </div>
        </div>
      </details>
...
async function addServer() {
  ...
  if (!name || !host) { alert('Server Name and Host/IP are required'); return; }
  const key = document.getElementById('srv_key').value.trim() || name.toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/_+$/, '');
...
function checkNightMode() {
  ...
  const nightLabel = document.getElementById('nightModeLabel');
  if (isNight && !window._nightModeActive) {
    window._nightModePrev = sel.value;
    window._nightModeActive = true;
    sel.value = '3600000';
    updateRefreshRate(3600000);
    if (nightLabel) nightLabel.style.display = 'inline';
  } else if (!isNight && window._nightModeActive) {
    window._nightModeActive = false;
    sel.value = window._nightModePrev || '3000';
    updateRefreshRate(parseInt(sel.value));
    if (nightLabel) nightLabel.style.display = 'none';
  }
}
